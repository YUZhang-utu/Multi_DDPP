...
class MoleculeDataset(torch.utils.data.Dataset):
    def __init__(self, smiles_list, targets, extra_features):
    ...
    def __getitem__(self, idx):
        ...
        return graph, torch.tensor(target, dtype=torch.float32), extra_features
...
def collate_fn(samples):
    graphs, targets, extra_features = zip(*samples)
    batched_graph = dgl.batch(graphs)
    targets = torch.stack(targets)
    extra_features = torch.stack(extra_features)

    return batched_graph, targets, extra_features
